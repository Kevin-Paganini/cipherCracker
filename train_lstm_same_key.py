...
parser.add_argument('--train_dataset_path', default='data/amazon_train.txt')
parser.add_argument('--test_dataset_path', default='data/amazon_test.txt')
...
if args.cipher == 'A':
    train_data = CryptDataset(AutokeyCipher(args.key), args.train_dataset_path)
    test_data = CryptDataset(AutokeyCipher(args.key), args.test_dataset_path)
elif args.cipher == 'C':
    train_data = CryptDataset(CaesarCipher(4), args.train_dataset_path)
    test_data = CryptDataset(CaesarCipher(4), args.test_dataset_path)
elif args.cipher == 'V':
    train_data = CryptDataset(VigenereCipher(args.key), args.train_dataset_path)
    test_data = CryptDataset(VigenereCipher(args.key), args.test_dataset_path)
else:
    raise NotImplementedError("Cipher not implemented")

# Create data loaders
train_data_loader = DataLoader(train_data, batch_size=500, shuffle=True)
